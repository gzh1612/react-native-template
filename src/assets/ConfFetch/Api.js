import {Fetch} from 'react-native-womasoft';
...
export default class Api {
  ...
  minerMeerBuy(id, count, payment, verifyCode, payType, code) {
    ...
    return new Fetch().post(
      url,
      {id, count, payment, verifyCode, payType, code},
      {isAuth: true},
    );
  }
  ...
  transfer(email, amount, assetsId, payment, code, memo, type) {
    ...
    return new Fetch().post(
      url,
      {email, amount, assetsId, payment, code, memo, type},
      {isAuth: true},
    );
  }
  ...
  withdraw(assetsId, payment, code, amount, address, memo) {
    ...
    return new Fetch().post(
      url,
      {assetsId, payment, code, amount, address, memo},
      {isAuth: true},
    );
  }
  ...
  easyChainBuy(easyChainId, verifyCode, Payment) {
    ...
    return new Fetch().post(
      url,
      {easyChainId, verifyCode, Payment},
      {isAuth: true},
    );
  }
